...
Our technique naturally lends itself to applications Phasor noise useful in, especially if only some artifact type are wanted. Figure \ref{fig:mountains} shows a mountain range generated using wobble and randomly placed singularities. It also show how a prescribed rip can be used mimic offset in the terrain created by an earthquake. Figure \ref{fig:desert} also makes use of a prescribed rip to emulate sand dunes, that form around a sheet of glass. Another interesting application of rips is shown in Figure \ref{fig:bricks} where a constructed Phasor noise instance is used to create detail in a texture, that respect the existing boundaries of the texture.
...
\begin{figure}[ht]
  ...
  \caption{Adding detail to a naively upscaled brick texture. The original texture (left) and a version with added naively detail (center). We can use rips to force discontinuities where the original texture suggests a change of material (right).}\label{fig:bricks}
\end{figure}

\section{Future Work}
Despite the demonstrated use cases for constructed artifacts in Phasor noise and the lack thereof, there are still areas to explore. One of these areas is the non-constant case. While the integrable case works fine, the non-integrable case still needs more work. The main limitation is the fact, that asymmetric oscillation profiles are no possible. On one hand we could try to keep track of where in the period we are, to enable asymmetric oscillation profiles, while on the other hand we could try to adapt the methods described here to work with symmetric oscillation profiles. Finding a good way to work with the work presented in \cite{stripes} also enables the application of this approach to work as surface noise. While this leads to a need of texture coordinates to specify where on the objects artifact should appear, these do not have to be free of discontinuities, as they would only be used to for this purpose. Therefore, even simple algorithms to generate texture coordinates will do.\\
Another avenue, that can be explored is an adaptation to work in higher dimensions, especially in an three dimensional environment. For the integrable case this is can be done by defining three dimensional equivalents to the artifact generation functions we defined here. The non-integrable case also requires the definition of a function, that can be used as a base for the oscillation.
